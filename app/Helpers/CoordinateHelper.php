...
{
    public static function utm2ll($x,$y,$zone,$aboveEquator){
		...
		$latlon = CoordinateHelper::UTMXYToLatLon($x, $y, $zone, $southhemi);
		return json_encode(array('success'=>true,'attr'=>array('lat'=>CoordinateHelper::radian2degree($latlon[0]),'lon'=>CoordinateHelper::radian2degree($latlon[1]))));
	}

	public static function ll2utm($lat,$lon){
		if(!is_numeric($lon)){
	...
	public static function UTMCentralMeridian($zone){
		$cmeridian = CoordinateHelper::degree2radian(-183.0 + ($zone * 6.0));
		return $cmeridian;
	}
	...
	public static function UTMXYToLatLon ($x, $y, $zone, $southhemi){
		$latlon = array();
		$UTMScaleFactor = 0.9996;
        	$x -= 500000.0;
	        $x /= $UTMScaleFactor;
	        ...
	        if ($southhemi)
        		$y -= 10000000.0;
        	$y /= $UTMScaleFactor;
	        $cmeridian = CoordinateHelper::UTMCentralMeridian ($zone);
        	$latlon = CoordinateHelper::MapXYToLatLon($x, $y, $cmeridian);
        	return $latlon;
	}
	public static function MapXYToLatLon ($x, $y, $lambda0){
		$philambda = array();
		$sm_b = 6356752.314;
		$sm_a = 6378137.0;
		$UTMScaleFactor = 0.9996;
		$sm_EccSquared = .00669437999013;
	        $phif = CoordinateHelper::FootpointLatitude ($y);
