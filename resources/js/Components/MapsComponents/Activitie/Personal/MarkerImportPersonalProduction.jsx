...
const MarkerImportPersonalProduction = ({ show, onHide }) => {
    ...
    const handleImportTowers = async (formData) => {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                toast.error(
                    "Erro ao importar Produções: " + response.statusText,
                );
            }
        } catch (error) {
            toast.error("Erro ao importar Produções: " + error.message);
        } finally {
    ...
    return (
        <Modal show={show} onHide={onHide}>
            <Modal.Header closeButton>
                <Modal.Title>Importar Produção</Modal.Title>
            </Modal.Header>
            <Modal.Body>
                <Form>
                    <Form.Group
...
export default MarkerImportPersonalProduction;
